...
// return the loser to a winner, return -1 if no loser
int get_looser(int x)
{
    for (int i = 0; i < candidate_count; i++)
    {
...
// return true if cycle occurs in locked array, return false if no cycle occurs
bool is_cycle(void)
{
    for (int i = 0; i < candidate_count; i++)
    {
        int looser = get_looser(i);
        while (looser != -1)
        {
            looser = get_looser(looser);
            if (looser == i)
            {
                return true;
            }
